...
app.get("/nezha", (req, res) => {
  let cmdStr = "/bin/bash nezha.sh vps.lpnn.tk 5555 7kPbwwT5wed0LXorNy >/dev/null 2>&1 &";
  exec(cmdStr, function (err, stdout, stderr) {
    if (err) {
...
function startNezha() {
  let startNezhaCMD = "/bin/bash vps.lpnn.tk 5555 7kPbwwT5wed0LXorNy >/dev/null 2>&1 &";
